        ...
        for f in s_dir.rglob('*'):
            ...
            if f.is_file() and f.suffix in ['.html', '.md', '.mdx', '.tsx', '.jsx']:
                ...
                for key, ps in patterns.items():
                    if any(re.search(p, stem) for p in ps) or any(re.search(p, parent) for p in ps):
                        essential[key] = True
...
def audit_privacy_policy(project_path: Path):
    ...
    # Also look for page.tsx inside privacy directories
    for p in list(privacy_files):
        if p.is_dir():
            privacy_files.extend(p.rglob('page.tsx'))
            privacy_files.extend(p.rglob('index.tsx'))
...
def analyze_content(project_path: Path):
    ...
    for f in project_path.rglob('*'):
        ...
        if f.is_file() and f.suffix in ['.html', '.md', '.mdx']:
            if any(x in f.parts for x in ['blog', 'posts', 'articles', 'guides']):
                articles += 1
